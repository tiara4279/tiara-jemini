...
# ═══════════════════════════════════════════════════════════
#  §11 심층 지표 분석 (프리미엄 패널 12개 연속 스크롤)
# ═══════════════════════════════════════════════════════════
...
def eval_detail(ticker, val, chg_1w):
    if ticker == "^VIX":
        ...
        if chg_1w < 0: return "🌊 유동성 지원", "#22D98A", f"역레포 잔고 {val:.2f}T — 잉여 현금이 RRP 시설에서 이탈하여 국채 및 민간 금융 시장으로 재유입되고 있습니다."
        else: return "🔒 유동성 회수", "#FF5555", f"역레포 잔고 {val:.2f}T — 민간 시장의 조달 매력도 저하로 인해 잉여 자금이 연준의 역레포 창구로 흡수되고 있습니다."
    elif ticker == "WRMFSL":
...
for selected_ind_name in ordered_list:
    ...
    with st.spinner(f"{selected_ind_name} 상세 데이터를 불러오고 있습니다..."):
        ...
        if series_detail is not None and not series_detail.empty:
            ...
            val_format = f"{cur_val:,.0f}" if meta["ticker"] == "WTREGEN" else f"{cur_val:,.2f}"
            prev_format = f"{val_1w:,.0f}" if meta["ticker"] == "WTREGEN" else f"{val_1w:,.2f}"
            chg_format = f"{abs(chg_1w):,.0f}" if meta["ticker"] == "WTREGEN" else f"{abs(chg_1w):,.2f}"
            m3_format = f"{val_3m:,.0f}" if meta["ticker"] == "WTREGEN" else f"{val_3m:,.2f}"
            y2_format = f"{val_2y:,.0f}" if meta["ticker"] == "WTREGEN" else f"{val_2y:,.2f}"
